  ...
  void _showGameEndModal(
    ...
    GameEndUnoModalWidget.show(
      ...
      onNewGame: () {
        ...
        // Сбрасываем флаг после закрытия модального окна
        _isGameEndModalShown = false;
      },
      onReturnToMenu: () {
        bloc.returnToMenu();
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<UnoBloc, UnoState>(
      listenWhen: (previous, current) {
        // Listen only to UnoGameState changes
        if (current is UnoGameState && previous is UnoGameState) {
          // If previous state already had gameEnded = true, don't trigger listener
          if (previous.gameEnded && current.gameEnded) {
            return false;
          }
        }
        return current is UnoGameState;
      },
        ...
        return Scaffold(
          appBar: CustomAppBar(
            ...
            onRightButtonPressed: () =>
                Navigator.pushNamed(context, '/unoRules'),
          ),
          body: SafeArea(
            child: LayoutBuilder(
